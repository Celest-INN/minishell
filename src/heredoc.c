...
/*   Updated: 2026/08/07 14:52:38 by ziyang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	open_heredoc(t_redir *redir, int *stop, t_pipex *pipex)
{
	...
	if (fd == -1)
		return (free(path), -1);
	r = heredoc_terminated_signal(fd, redir->file, pipex);
	if (r == -1)
		return (free(path), -1);
	if (r != 0)
	{
		*stop = 1;
		(ft_putchar_fd('\n', STDOUT_FILENO), rl_on_new_line());
		return (r);
	}
	close(fd);
	free(redir->file);
...
int	all_heredoc(t_argv *cmd, t_pipex *pipex)
{
	...
	while (cmd)
	{
		if (open_heredoc(cmd->redir, &stop, pipex) != 0)
			return (-1);
		cmd = cmd->next;
	}
	return (0);
	...
	while (cmd != NULL)
	{
		...
		while (redir)
		{
			if (redir->type == HEREDOC && (ft_strncmp(redir->file,
						"/var/tmp/.heredoc-", 18) == 0) && (access(redir->file,
						R_OK) == 0))
				unlink(redir->file);
			redir = redir->next;
		}
